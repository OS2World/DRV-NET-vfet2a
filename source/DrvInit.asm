...
strKeyword16	db	'NETADDRESS',0
lenKeyword16	equ	$ - offset strKeyword16
strKeyword17	db	'OPENBIND',0
lenKeyword17	equ	$ - offset strKeyword17
strKeyword18	db	'IRQ15OVR',0
...
Credit		db	cr,lf,' VIA VT6102 Rhine II Another OS/2 NDIS MAC Driver '
		db	'ver.1.06 (2005-12-17)',cr,lf,0
Copyright	db	0	; Write copyright message here if you want.

Heap		db	( 84*9*(sizeof(TD) +8) \
			+ 84*sizeof(vtxd) \
			+ 84* sizeof(RD) \
			+ 4096 ) dup (?)
...
loc_1:
	...
	cmp	eax,30651106h		; VT6102
	jnz	short loc_3

	push	word ptr [FH_bdf]
	call	QueryChipRev		; [40h..42h]
	pop	cx	; stack adjust
IF 1			; enable/disable chip revision check.
	cmp	al,40h
	jc	short loc_3
	cmp	al,7fh			; [40h..7fh?]
	ja	short loc_3
...
_AllocMemBlock	proc	near
	...
	div	cx
	neg	dx
	adc	ax,1		; request GDT selector count, +1 for TxCopySel
